...
public class MvcControllerC {
   ...
   * 
   * 1. 컨트롤러가 View(JSP)로 데이터를 전달할 때 사용하는 인터페이스입니다.
   ...
   *    2) 파라미터 메소드 : 요청 파라미터 (또는 폼 데이터)를 자바 객체에 자동으로 바인딩합니다.
   */
  //-----
...
//------@ModelAttribute를 이용한 forward (데이터 전달)
  ...
  public String submit(@ModelAttribute("board") BoardDTO board) {  //------- Model에 board라는 이름으로 BoardDTO board 객체가 저장됩니다. (Model.addAttribute를 안해도 된다) => JSP에서 ${board} 확인
      return "c/detail";   // ------------- forward jsp이름        //------- name = "board를 생략하면 (@ModelAttribute BoardDTO board) Model에 boardDTO 라는 이름으로 BoardDTO board 객체가 저장됩니다. => JSP에서 ${boardDTO} 확인
                                                                   //-------- Model Attribute를삭제할 수 있습니다. (스프링 3.2 이상) 
  }   
   ...
   * 2. 리다이렉트 시 Model에 저장한 데이터는 소멸되므로 (새로운 요청이기 때문에) RedirectAttributes를 사용해야합니다. 
   * 3. 주요 메소드
   *    1) addAttribute(String key, Object value)
  ...
  //----- RedirectAttributes 를 이용한 redirect
  @RequestMapping("/modify.do")  //---- 요청주소 : /c/modify?bid=0&title=수정제목&hit=10
  ...
  // 모든 요청 주소를  .do 방식으로 수정해봅시다.
  // return "c/list" ;  jsp경로
  // return "redirect:/c/list"  ; redirect 주소  
  //  jsp 이름을 적어주는 건지 주소를 적어주는건지 구분
