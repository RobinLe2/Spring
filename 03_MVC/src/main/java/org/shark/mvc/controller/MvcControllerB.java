...
public class MvcControllerB {
  ...
  public String methodA(HttpServletRequest request) {
    ...
    //------ page 값이 정수가 아닌 경우 (전달 안 된 경우도 포함) "1" 값을 사용합니다
    int page = 1;
    try {
  ...
  public String methodB(@RequestParam(name = "bid") int bid     //------ @RequestParam 어노테이션으로 요청 파라미터를 처리할 수 있습니다.
    ...
    // 1. @RequsetParam 어노테이션을 명시하는 경우 @RequestParam(required = true)가 디폴트 설정이므로 파라미터를 전달하지 않으면 400dpfj
  ...
  public String methodC(BoardDTO board) {  //------ 커멘드 객체를 이용한 요청 파라미터 처리: Constructor 또는 Setter를 이용해서 요청 파라미터를 BoardDTO의 멤버 변수(필드)에 저장합니다.
