 ...
 * 
 * 1. 클라이언트가 "/d/*" 요청을 보냅니다.
 * 2. 세션에 boards가 없으면 getBoards() 메소드가 실행되어 BoardList 객체가 만들어지고 Model에 boards란 이름으로 저장됩니다.
 ...
 * 5. 요청 파라미터 title과 hit가 BoardDTO board regist() 메소드의  파라미터 BoardList boardList에 바인딩됩니다.
 * 6. regist() 메소드의 본문에서 board가 boardList에 저장되고 , 목록보기를 요청합니다.
 * 7. 목록보기 요청(/d/list) 시 /WEB-INF/views/d/list.jsp로 이동합니다.
 * 8. list.jsp 에서 boards 값을 확인합니다.
 */
...
public class MvcControllerD {
  ...
  public String regist(@ModelAttribute(name = "boards") BoardList boardList  //---- 3. 세션에 있는 boards 를 BoardList boardList로 가져옵니다.
                       , BoardDTO board) {                                    //----    세션에 boards가 없으면 컨트롤러의 @ModelAttribute("boards") 메소드(getBoards())가 호출되어 새로운 BoardList 객체를 세션에 저장합니다.
  ...
  public String method(SessionStatus status) {
